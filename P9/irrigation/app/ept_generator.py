...
ev = [(i.id, str(i.name), i.rules_id, i.actions_id) for i in events]
ru = [db_session.query(Rules).filter(Rules.id==i[2]).first() for i in ev]
ac = [db_session.query(Actions).filter(Actions.id==i[3]).first() for i in ev]

r = [str(i.expression) for i in ru]
a = [str(i.expression) for i in ac]

# Atribuindo um label para cada regra e cada ação
rules = {}
for i in ru:
	rules[str(i.name)] = str(i.expression)

actions = {}
for i in ac:
	actions[str(i.name)] = str(i.expression)
...
# events = [[rule_name_expr[0][0], a[0]], [rule_name_expr[1][0], a[1]]]
events = [[str(ru[i].name), str(ac[i].expression)] for i in range(len(ev))]
